...
ITEMS_TO_SYNC = [
    # Explicit user list: Albums & Songs
    {"query": "Nirvana Nevermind", "type": "album", "price": 34.99, "stock": 25},
    {"query": "Nirvana MTV Unplugged In New York", "type": "album", "price": 32.99, "stock": 20},
    {"query": "Joji Ballads 1", "type": "album", "price": 29.99, "stock": 30},
    {"query": "Joji SMITHEREENS", "type": "album", "price": 28.99, "stock": 30},
    {"query": "Joji In Tongues", "type": "album", "price": 24.99, "stock": 20},
    {"query": "Joji SLOW DANCING IN THE DARK", "type": "track", "price": 14.99, "stock": 15},
    {"query": "Joji Glimpse of Us", "type": "track", "price": 14.99, "stock": 15},
    {"query": "Joji Pixelated Kisses", "type": "track", "price": 12.99, "stock": 10},
    {"query": "Joji Piss in the Wind", "type": "track", "price": 12.99, "stock": 10},
    {"query": "Joji Beautiful", "type": "track", "price": 12.99, "stock": 10},
    ...
    {"query": "The Script #3", "type": "album", "price": 27.99, "stock": 15},
    {"query": "Radiohead OK Computer", "type": "album", "price": 34.99, "stock": 25},
    ...
    # Selected iconic tracks from the user's uploaded Spotify playlists
        ...
        try:
            ...
            if expected_type == "album":
                ...
                if albums:
                    ...
                    synced = await sync_service.sync_album_by_spotify_id(
                        spotify_album_id=album_id,
                        default_price=price,
                        default_stock=stock,
                    )
                    results.append({"query": query, "synced_type": "album", "name": album_name, "id": album_id})
                else:
                    logger.warning(f"No album match found for query: {query}")
            else:
                ...
                if tracks:
                    ...
                    synced = await sync_service.sync_track_by_spotify_id(
                        spotify_track_id=track_id,
                        default_price=price,
                        default_stock=stock,
                    )
                    results.append({"query": query, "synced_type": "track", "name": track_name, "id": track_id})
                else:
                    logger.warning(f"No track match found for query: {query}")
        except Exception as e:
            logger.error(f"Error syncing '{query}': {e}", exc_info=True)
            
    logger.info(f"Completed syncing {len(results)} items into MongoDB Atlas!")
    await close_mongo_connection()
    return results


if __name__ == "__main__":
    asyncio.run(sync_all_items())
